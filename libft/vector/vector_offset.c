...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   vector_add_element.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lcarmelo <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/07 14:36:57 by lcarmelo          #+#    #+#             */
/*   Updated: 2020/02/07 17:10:11 by lcarmelo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void            _vector_offset(t_vector *vector, size_t index, void *element, t_uc offset_flag)
{
    ...
    {
        ft_memmove(vector_offset(vector, index + 1), vector_offset(vector, index), vector->element_size * (vector->size - index));
        ft_memcpy(vector_offset(vector, index), element, vector->element_size);
    }
    else if (offset_flag == OFFSET_LEFT)
        ft_memmove(vector_offset(vector, index), vector_offset(vector, index + 1), vector->element_size * (vector->size - index)); 
}
